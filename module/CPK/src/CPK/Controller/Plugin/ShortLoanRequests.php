<?php
/**
 * VuFind Action Helper - Short loan requests
 *
 * PHP version 5
 *
...
/**
 * Zend action helper to manage short loan requests
 *
 * @category VuFind2
 * @package  Controller_Plugins
 ...
 * @link     http://www.vufind.org  Main Page
 */
class ShortLoanRequests extends AbstractRequestBase
{
     ...
     */
    public function addCancelDetails($catalog, $ilsDetails)
    {
        $ilsDetails['cancel_details'] = $catalog->getCancelShortLoanRequestDetails($ilsDetails);
    ...
    public function cancelShortLoanRequests($catalog, $patron)
    {
        ...
        if (!empty($details)) {
            ...
            $cancelResults = $catalog->cancelShortLoanRequest(
