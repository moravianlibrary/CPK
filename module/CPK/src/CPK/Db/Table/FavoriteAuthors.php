<?php
/**
 * Table Definition for FavoriteAuthors
 *
 * PHP version 5
 *
 ...
 * @link     http://vufind.org Main Site
 */
class FavoriteAuthors extends Gateway
{
    /**
    ...
    {
        $this->config = $config;
        parent::__construct('favorite_authors', 'CPK\Db\Row\FavoriteAuthors');
    }
    ...
    public function saveRecords($records)
    {
        ...
        foreach($records as $record) {
            ...
            $shortNoteCs = $record->getShortNoteCs();
            $shortNoteEn = $record->getShortNoteEn();
            $authorityName = $record->getHeading();

            $authorityId = $record->getUniqueID();

            $arrayOfValues = [
                'authority_id' => $authorityId,
                'authority_name' => $authorityName,
                'short_note_cs' => $shortNoteCs,
                'short_note_en' => $shortNoteEn
            ];
